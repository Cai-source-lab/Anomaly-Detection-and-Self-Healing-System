...
public class cy_HyDao {
    ...
    public List<cy_Hy> QuerySomehy(String sql, Object[] param) {
        List<cy_Hy> somehy = new ArrayList<>();
        MysqlConn db = new MysqlConn();
        ResultSet rs = null;
        try {
            rs = db.doQuery(sql, param);
    ...
    // 4. 查询单个会员信息
    public cy_Hy getMemberById(int memberId) {
        System.out.println("cy_HyDao.getMemberById: 开始查询会员ID = " + memberId);
        cy_Hy member = null;
        MysqlConn db = new MysqlConn();
        ResultSet rs = null;
        
        String sql = "SELECT * FROM cy_vip WHERE id = ?";
        Object[] params = {memberId};
        
        try {
            System.out.println("cy_HyDao.getMemberById: 执行SQL查询");
            rs = db.doQuery(sql, params);
            if (rs != null && rs.next()) {
                System.out.println("cy_HyDao.getMemberById: 找到会员数据，开始封装");
                member = new cy_Hy();
                ...
                System.out.println("cy_HyDao.getMemberById: 会员数据封装完成 - " + member.getVname());
            } else {
                System.out.println("cy_HyDao.getMemberById: 没有找到会员数据");
            }
        } catch (Exception e) {
            System.out.println("cy_HyDao.getMemberById: 查询单个会员异常：" + e.getMessage());
            e.printStackTrace();
        } finally {
            db.close();
        }
        System.out.println("cy_HyDao.getMemberById: 返回结果 = " + (member != null ? "找到会员" : "未找到会员"));
        return member;
    }
     ...
     * 根據電話號碼查詢會員信息
     */
    public cy_Hy getMemberByPhone(String phone) {
        ...
        MysqlConn db = new MysqlConn();
        ResultSet rs = null;
        String sql = "SELECT * FROM cy_vip WHERE phone = ?";
        Object[] params = {phone.trim()};
        try {
            rs = db.doQuery(sql, params);
            if (rs != null && rs.next()) {
                member = new cy_Hy();
            ...
            System.out.println("根據電話號碼查詢會員異常：" + e.getMessage());
            e.printStackTrace();
        } finally {
            db.close();
        }
    ...
    // 5. 无用方法（保留但无需修改）
    public List<cy_Hy> doQuery(String s, Object[] objects) {
